/**
 * In the 20×20 grid below, four numbers along a diagonal line have been marked in red. (See ProjectEuler)
			...
			for (int j = 0; j <= 19; j++) {
				...
				while (three >= 0) {
					if (((i + three) <= 19) && ((j + three) <= 19 )) {
						productSE *= array[i + three][j + three];
					}
					three--;
				}
				...
				while (three >= 0) {
					if (((i + three) <= 19) && ((j + three) <= 19 ) && ((j - three) >= 0)) {
						productSW *= array[i + three][j - three];
					}
					three--;
				}
